...
- Use `BaseCommand` when the command participates in the JSON and central error contracts.
- Keep non-interactive behavior as the default. When both modes exist, `--interactive` requires a
  TTY and cannot be combined with `--json`.
