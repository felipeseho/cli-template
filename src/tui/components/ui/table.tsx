...
>({
  ...
  const visibleStart = Math.max(
    0,
    Math.min(activeRow - maxRows + 1, sorted.length - maxRows)
  );
  const visible = sorted.slice(visibleStart, visibleStart + maxRows);
  ...
  const { isFocused } = useInteraction(
    (input, key) => {
      ...
      if (key.upArrow) {
        ...
        setActiveRow((r) => Math.min(sorted.length - 1, r + 1));
      } else if (key.home) {
        setActiveRow(0);
      } else if (key.end) {
        setActiveRow(sorted.length - 1);
  ...
  useEffect(() => {
    setActiveRow((row) => Math.min(row, Math.max(0, sorted.length - 1)));
  }, [sorted.length]);
  ...
  return (
    ...
    >
      ...
      {isFocused && selectable && sorted[activeRow] && (
        <Text
          aria-label={`Selected row ${activeRow + 1}: ${columns
            .map(
              (column) =>
                `${column.header}: ${String(sorted[activeRow]?.[column.key] ?? "")}`
            )
            .join(",")}`}
        >
          {""}
      ...
      <Box aria-hidden flexDirection="column">
        ...
        {sorted.length > maxRows &&
          (() => {
            ...
            const msg = `${unicode ? "…" : "..."} rows ${visibleStart + 1}-${visibleStart + visible.length} of ${sorted.length}`;
            return (
              <Box flexDirection="row">
      ...
      {visible.map((row, rowIdx) => (
        <Text
          ...
          aria-label={`Row ${visibleStart + rowIdx + 1}: ${columns
            .map(
