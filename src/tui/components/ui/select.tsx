...
export const Select = <T = string,>({
  ...
  const requestedVisible = maxVisibleOptions ?? Math.max(1, options.length);
  const visibleCount = Math.max(1, Math.floor(requestedVisible));
  const maximumWindowStart = Math.max(0, options.length - visibleCount);
  ...
  const visibleOptions = options
    ...
    .map((option, offset) => ({ index: windowStart + offset, option }));

  const resolvedCursorColor = cursorColor ?? theme.colors.primary;

  useEffect(() => {
    setActiveIndex((current) =>
      Math.max(0, Math.min(current, Math.max(0, options.length - 1)))
    );
  }, [options.length]);
  ...
  return (
    ...
    >
      {label && (
        <Text aria-label={ariaLabel ?? label} bold wrap="truncate-end">
          {label}
        </Text>
      )}
      ...
      {options.length > visibleCount && (
        <Text color={theme.colors.mutedForeground} dimColor wrap="truncate-end">
          {windowStart + 1}-{Math.min(windowStart + visibleCount, options.length)}/
          {options.length}
        </Text>
      )}
    </Box>
